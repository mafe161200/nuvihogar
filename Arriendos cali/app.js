...
function obtenerDatosColor(canon) {
    if (!canon || canon === 0 || promedioGlobal === 0) return { color: "#95a5a6", borde: "#7f8c8d" };
    if (canon <= limiteBaratoGlobal) return { color: "#2ecc71", borde: "#27ae60" };
    if (canon >= limiteCaroGlobal) return { color: "#e74c3c", borde: "#c0392b" };
...
function actualizarMapa() {
    ...
    inmueblesMapeables.forEach(inmueble => {
        ...
        const marcador = L.circleMarker([lat, lng], {
            radius: 8, fillColor: colorInfo.color, color: colorInfo.borde, weight: 2, opacity: 1, fillOpacity: 0.85
        });
    ...
    if (bounds.length > 0) mapa.fitBounds(bounds, { padding: [20, 20], maxZoom: 14 });
}
...
window.irATarjeta = function(idInmueble) {
    ...
    if (index !== -1) {
        ...
        if (paginaActual !== paginaDestino) { paginaActual = paginaDestino; mostrarInmuebles(); }
        setTimeout(() => {
            ...
            if (tarjetaElemento) {
                ...
                setTimeout(() => { tarjetaElemento.classList.remove('tarjeta-destacada'); }, 2000);
            }
        }, 150);
    }
};
...
function limpiarCodificacion(valor) {
    ...
    const reemplazos = { "Ã¡": "á", "Ã©": "é", "Ã­": "í", "Ã³": "ó", "Ãº": "ú", "Ã±": "ñ", "Ã ": "Á", "Ã‰": "É", "Ã ": "Í", "Ã“": "Ó", "Ãš": "Ú", "Ã‘": "Ñ" };
...
function precioCOP(valor) { if (!valor || Number(valor) <= 0) return "Consultar"; return `$${Number(valor).toLocaleString("es-CO", { maximumFractionDigits: 0 })}`; }
function convertirBooleano(valor) { return (valor === true || valor === "true" || valor === "si"); }
function coincideBooleano(valor, filtro) { if (!filtro) return true; const booleano = convertirBooleano(valor); return filtro === "si" ? booleano : !booleano; }
...
window.toggleFavorito = function(idInmueble, elementoBoton) {
    ...
    if (index === -1) {
        favoritos.push(id); elementoBoton.classList.add("activo"); icono.innerHTML = "♥"; 
        elementoBoton.setAttribute("aria-label", "Quitar de favoritos"); elementoBoton.setAttribute("aria-pressed", "true");
    } else {
        favoritos.splice(index, 1); elementoBoton.classList.remove("activo"); icono.innerHTML = "♡"; 
        elementoBoton.setAttribute("aria-label", "Guardar en favoritos"); elementoBoton.setAttribute("aria-pressed", "false");
        if (viendoSoloFavoritos) aplicarFiltros();
    }
    localStorage.setItem("mis_favoritos_arriendos", JSON.stringify(favoritos));
};

function obtenerValoresUnicos(campo) { return [...new Set(inmuebles.map(i => limpiarCodificacion(i[campo])).filter(v => v && v.trim() !== ""))].sort(); }
...
function llenarSelect(id, valores, textoInicial) {
    ...
    valores.forEach(v => { const op = document.createElement("option"); op.value = v; op.textContent = v; select.appendChild(op); });
...
function aplicarFiltros() {
    ...
    ordenarResultados(); actualizarEstadisticas(); mostrarInmuebles(); actualizarMapa();
}
...
function actualizarEstadisticas() {
    ...
    if (!precios.length) {
        ...
        establecerTexto("estadistica-promedio", "$0"); establecerTexto("estadistica-minimo", "$0"); establecerTexto("estadistica-maximo", "$0");
    } else {
...
function ordenarResultados() {
    ...
    resultadosActuales.sort((a, b) => {
        ...
        switch (orden) {
            case "precio-menor": return pA - pB; case "precio-mayor": return pB - pA;
            case "area-mayor": return (Number(b.area_m2) || 0) - (Number(a.area_m2) || 0); default: return 0;
        }
    });
}
...
function crearTarjeta(inmueble) {
    ...
    tarjeta.className = "property-card"; tarjeta.id = `card-${inmueble.id}`;
    ...
    const overlayHTML = usaImagenReferencia ? `<div class="overlay-referencia"><span class="texto-ref">Imagen de ref.</span></div>` : '';
...
function mostrarPaginacion() {
    ...
    btnPrev.innerHTML = "&laquo;"; btnPrev.className = "arrow-btn"; btnPrev.title = "Página anterior"; btnPrev.disabled = paginaActual === 1;
    btnPrev.onclick = () => { if (paginaActual > 1) { paginaActual--; mostrarInmuebles(); window.scrollTo({ top: 0, behavior: "smooth" }); } };
    ...
    for (let p = inicio; p <= fin; p++) {
        ...
        boton.addEventListener("click", () => { paginaActual = p; mostrarInmuebles(); window.scrollTo({ top: 0, behavior: "smooth" }); });
        contenedor.appendChild(boton);
    }

    const btnNext = document.createElement("button");
    btnNext.innerHTML = "&raquo;"; btnNext.className = "arrow-btn"; btnNext.title = "Página siguiente"; btnNext.disabled = paginaActual === totalPaginas;
    btnNext.onclick = () => { if (paginaActual < totalPaginas) { paginaActual++; mostrarInmuebles(); window.scrollTo({ top: 0, behavior: "smooth" }); } };
